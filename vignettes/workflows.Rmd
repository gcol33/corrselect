...
This vignette demonstrates complete end-to-end workflows using `corrselect` across four different research domains:

1. **Ecological Modeling**: Predicting species distribution with bioclimatic variables
2. **Survey Data Analysis**: Analyzing questionnaires with redundant items
3. **High-Dimensional Data**: Gene expression analysis
4. **Mixed Models**: Longitudinal studies with random effects
...
You're building a species distribution model using 50 WorldClim bioclimatic variables. Many variables are highly correlated (e.g., different temperature metrics), causing multicollinearity.

**Goal**: Identify a parsimonious set of predictors that:
- Covers different environmental dimensions
- Has low multicollinearity
- Predicts species richness well
...
## Step 4: VIF cleanup with `modelPrune()`
...
comparison <- data.frame(
  Model = c("Full (50 vars)", "After corrPrune (15 vars)", "After modelPrune (8 vars)"),
  R2 = c(
...
**Interpretation**:
- R² decreased slightly (expected - we removed predictors)
- **Adjusted R² improved** (better parsimony)
- **AIC improved** (better model fit penalized for complexity)
- **Multicollinearity eliminated** (all VIF < 5)
...
## Step 2: Prune redundant items with `force_in`

We want to:
- Remove redundant Likert items (correlation > 0.6)
- **Always keep** age (key covariate)
...
## Step 3: Verify construct coverage
...
## Step 5: Compare with full model
...
**Key insight**: Similar predictive power with 70% fewer items - much more practical for future surveys!
...
**Key insight**: Greedy mode is faster and produces solutions close to optimal.
...
# Workflow 4: Mixed Models (Longitudinal Data)
...
**Goal**: Remove multicollinear fixed effects using `modelPrune()` with `lme4` engine.
...
## Step 2: Prune fixed effects with mixed model engine
...
**Key insight**: Random effects `(1|subject)` and `(1|site)` were preserved - only problematic fixed effects were removed.
...
|--------|------------------|-------------|
| **Ecology** | `corrPrune()` → `modelPrune()` | Two-step pruning for dimensionality reduction |
| **Survey Research** | `corrPrune()` with `force_in` | Protect key covariates while removing redundancy |
| **Genomics** | `corrPrune(mode = "greedy")` | Fast pruning for high-dimensional data |
| **Longitudinal Studies** | `modelPrune(engine = "lme4")` | Prune fixed effects, preserve random effects |

## Choosing thresholds

### For `corrPrune()` (correlation threshold)

- **0.5**: Strict (low redundancy, may lose information)
- **0.7**: Moderate (recommended starting point)
- **0.9**: Lenient (only remove near-duplicates)

### For `modelPrune()` (VIF limit)

- **5**: Standard threshold (moderate multicollinearity)
- **10**: Lenient (tolerate more multicollinearity)
- **2**: Strict (very low multicollinearity, may over-prune)

**Pro tip**: Visualize correlation/VIF distributions before choosing!
...
**Use when**: You have many predictors (p > 30) and want computational efficiency.
...
pruned <- corrPrune(data, threshold = 0.6,
                    force_in = c("age", "gender", "treatment"))
```

**Use when**: Some predictors must remain for scientific/policy reasons.
...
**Solution**: Increase threshold or use `mode = "greedy"` for best-effort solution.

## Problem: Too many variables removed

**Cause**: Threshold too lenient, or limit too low.

**Solution**:
- Decrease correlation threshold (e.g., 0.7 → 0.8)
- Increase VIF limit (e.g., 5 → 10)
- Use `force_in` to protect important variables

## Problem: Pruning takes too long

**Cause**: Using `mode = "exact"` with large p.

**Solution**: Use `mode = "greedy"` or `mode = "auto"` (auto-switches at p > 20).
...
- **Custom engines**: See `?modelPrune` for integrating custom modeling packages (INLA, mgcv, etc.)
- **Function reference**: Full documentation at `?corrPrune` and `?modelPrune`
