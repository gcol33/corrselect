...
benchmark_corrPrune <- function(p_values) {
  ...
  for (p in p_values) {
    ...
    # Exact mode (benchmark all p values)
    exact_time_ms <- median(microbenchmark(
...
**Key insight**: Exact mode shows exponential scaling behavior, while greedy mode scales approximately linearly. Both modes remain practical up to p = 300 for typical correlation structures, with exact mode providing guaranteed optimality and greedy mode offering faster execution.
