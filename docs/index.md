...
[![Monthly
downloads](https://cranlogs.r-pkg.org/badges/corrselect)](https://cran.r-project.org/package=corrselect)
...
**Exact enumeration of maximal predictor sets under a correlation
threshold.**

Hand it a data frame and a threshold. `corrselect` finds every maximal
set of predictors whose pairwise correlations all stay under that
threshold, by graph enumeration (Bron–Kerbosch /
Eppstein–Löffler–Strash) in C++, and returns whichever view you need: a
single pruned data frame, or every valid subset to choose from.
...
returns every maximal low-correlation subset, ranked by size then
average correlation:
...
`force_in` protects variables that must survive pruning regardless of
what else gets dropped; `by` requires the threshold to hold separately
inside every group:
...
runs the same mixed-type dispatch under the hood, so a single pruning
call works whether the input is all-numeric or a mix of numeric, factor,
and ordered columns. Six numeric-numeric measures are available,
...
removes predictors by VIF or condition number, refitting after each
removal. Built-in engines cover `lm`, `glm`, `lme4`, and `glmmTMB`;
...
|  | [`corrPrune()`](https://gillescolling.com/corrselect/reference/corrPrune.md) | [`modelPrune()`](https://gillescolling.com/corrselect/reference/modelPrune.md) |
|----|----|----|
...
Use
[`corrPrune()`](https://gillescolling.com/corrselect/reference/corrPrune.md)
first to cut dimensionality, then
[`modelPrune()`](https://gillescolling.com/corrselect/reference/modelPrune.md)
for final cleanup inside a modeling framework.

## Exact where it matters, greedy where it scales

[`corrPrune()`](https://gillescolling.com/corrselect/reference/corrPrune.md)
picks a search mode for you: exact enumeration up to `max_exact_p`
predictors (100 by default), and a greedy C++ backend beyond that. Exact
mode guarantees the largest subset satisfying the threshold; greedy mode
trades that guarantee for speed on wide data. Both are available
directly, so you can pin one explicitly:
...
solves this with greedy iterative removal, which is order-dependent and
can vary between runs on the same data. `corrPrune(mode = "exact")`
solves the same threshold constraint by maximal-clique enumeration,
retaining at least as many variables and returning the same answer every
run. The [Comparison
vignette](https://gillescolling.com/corrselect/articles/comparison.html)
walks through this and three other alternatives (Boruta, glmnet, manual
VIF removal) side by side on the same dataset.

## What’s in the box

- **[`corrPrune()`](https://gillescolling.com/corrselect/reference/corrPrune.md)**:
  association-based pruning, model-free. Exact mode for `p <= 100`,
  greedy mode for larger `p`, protect variables with `force_in`, hold
  the threshold across groups with `by`.
- **[`modelPrune()`](https://gillescolling.com/corrselect/reference/modelPrune.md)**:
  VIF-based pruning for `lm`, `glm`, `lme4`, `glmmTMB`, or any custom
  engine (INLA, mgcv, brms, …).
- **[`corrSelect()`](https://gillescolling.com/corrselect/reference/corrSelect.md)
  /
  [`MatSelect()`](https://gillescolling.com/corrselect/reference/MatSelect.md)**:
  exhaustive enumeration of all maximal sets, on a data frame or
  directly on a correlation matrix.
- **[`corrSubset()`](https://gillescolling.com/corrselect/reference/corrSubset.md)**:
  pull one or more maximal subsets back out of a `CorrCombo` as data
  frames.
- **[`assocSelect()`](https://gillescolling.com/corrselect/reference/assocSelect.md)**:
  mixed-type data (numeric, factor, ordered), with the right association
  metric chosen per pair.
...
- [Quick
  Start](https://gillescolling.com/corrselect/articles/quickstart.html)
...
- [Advanced
  Usage](https://gillescolling.com/corrselect/articles/advanced.html)
- [Theory](https://gillescolling.com/corrselect/articles/theory.html)
- [Function
  Reference](https://gillescolling.com/corrselect/reference/index.html)
